...
$trigger.on('click', function () {
  ...
  if (expanded === 'false') {
    ...
    $gnav.attr('aria-hidden', false).fadeIn(300); //メニューのhiddenステートをfalseにしてメニューを表示
    ...
    $gnav.attr('aria-hidden', true).fadeOut(300); //メニューのhiddenステートをtrueにしてメニューを閉じる
    $('body').toggleClass('is-active');
  }
});
...
//     $gnav.attr('aria-hidden', false).show(); //グロナビを表示
//   } else {
//     //スマホレイアウトの初期状態にリセット
...
// $(point_header).on('change', checkBreakPoint);
...
$(function () {
  ...
  $(document).on('click', 'a[href*="#"]', function () {
    ...
    const $targetY = $target.offset().top - $headerHight;
  ...
  if ($urlHash) {
    ...
    setTimeout(function () {
      ...
      const $targetY = $target.offset().top - $headerHight;
      $('body,html').stop().animate({ scrollTop: $targetY }, $time, 'swing');
    }, 100);
  }
});
