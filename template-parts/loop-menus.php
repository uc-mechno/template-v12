...
// $allowed_html = ['br' => []];
$fields = CFS()->get($args['menu-name']);
$limit_image = wp_get_attachment_image_src(CFS()->get('limit_image'), 'full');
  ...
  foreach ($fields as $field) {
    ...
    <li class="p-menus-sec__item">
      <div class="p-menus-sec__img js-blurTrigger is-show">
        ...
        <?php if (!$field['limit_if'] && $limit_image) { ?>
          <div class="p-menus-sec__limit">
            <img width="<?php echo esc_attr($limit_image[1]); ?>" height="<?php echo esc_attr($limit_image[2]); ?>" loading="lazy" decoding="async" src="<?php echo esc_attr($limit_image[0]); ?>">
          </div>
        <?php } ?>
